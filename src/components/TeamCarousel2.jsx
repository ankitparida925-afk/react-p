...
function TeamCarousel() {
  ...
  // Frame-by-frame auto-rotation using JS animation to prevent CSS wrap swooping
  useEffect(() => {
    ...
    const startSnap = () => {
      ...
      const duration = 600; // 600ms smooth slide

      const animateSnap = (time) => {
        // Grab the starting index on the very first frame
        if (startIdx === null) {
          setActiveIndex(prev => {
            startIdx = prev;
            // Target exactly 1 full card forward from the nearest slot
            targetIdx = Math.round(prev) + 1;
            return prev;
          });
        }
        ...
        setActiveIndex(() => {
           let next = startIdx + (targetIdx - startIdx) * easeProgress;
           // Keep index cleanly wrapped within the array bounds
           while (next >= teamMembers.length) next -= teamMembers.length;
           while (next < 0) next += teamMembers.length;
           return next;
        });
        ...
        if (progress < 1) {
          ...
          timeoutId = setTimeout(startSnap, 2250);
        }
      };
